...
This project uses a specific backend architecture:

- Database: PostgreSQL (Neon)
- Query layer: sqlc (ALL queries must be defined in sqlc/queries)
...
- NEVER write raw SQL inside Go code
- ALWAYS create queries inside sqlc/queries
- ALWAYS use sqlc generated code for database access
- Follow layering:
  handler → service → repository (via sqlc)

### Feature Development Flow

When implementing features:

1. Define SQL queries first (sqlc)
2. Assume sqlc generates types and methods
3. Implement repository using sqlc
4. Implement service layer
5. Implement HTTP handler

Ignore generic repository patterns if they conflict with sqlc usage.
