...
func TestService_DeleteProperty(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			storageKeys:            []string{},
			input:                  DeletePropertyInput{Actor: ActorContext{UserID: 1, RoleID: RoleAdminID}, Confirm: true, ChangedByUserID: changedByUserID},
			wantErr:                false,
			wantRepoCalled:         true,
		...
		{
			name: "logs cleanup failure when deleting property photos from storage fails",
			propertyResult: GetPropertyResult{
			...
			wantErr:                false,
			wantRepoCalled:         true,
			wantStorageDeleteCalls: 2,
			wantDeleteArgs:         deleteArgs{propertyID: 16, changedByUserID: changedByUserID},
		},
	}
