...
func TestService_ListProperties_CU12(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			repoItems: []PropertyCardData{{
				PropertyUUID:  "b",
				CoverPhotoURL: ptrString("properties/b/photos/cover.webp"),
				AssignedAgent: &PropertyAgentData{
					UserID:    9,
					UserUUID:  "agent-uuid-2",
					FirstName: "Grace",
					LastName:  "Hopper",
				},
			}},
...
func TestService_GetPropertyForRole(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			userID:           10,
			roleID:           RoleAdminID,
			repoResult:       GetPropertyResult{Data: GetPropertyData{PropertyID: 5, PropertyUUID: "uuid", RegisteredBy: "Admin User", AssignedAgent: &PropertyAgentData{UserID: 21}}},
		...
		},
		{
			...
			userID:           20,
			roleID:           RoleAgentID,
			repoResult:       GetPropertyResult{Data: GetPropertyData{PropertyID: 5, PropertyUUID: "uuid", RegisteredBy: "Admin User", AssignedAgent: &PropertyAgentData{UserID: 20}}},
			assigned:         true,
			wantRegisteredBy: "",
		},
		{
